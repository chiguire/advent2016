module Advent4
    ( advent4_1, advent4_2, deriveChecksum
    ) where
...
deriveChecksum s = map (snd) $ take 5 $ sortBy (cmp) $ (map (length &&& head)) $ group $ sort s

parseSectorIdChecksum :: String -> (Int, String)
parseSectorIdChecksum s = (read sid, reverse $ drop 1 $ reverse $ drop (length sid + 1) s)
    where sid = takeWhile (isDigit) s

parseString :: String -> Room
parseString s = (Room roomname sectorid checksum)
    where splitString = splitOn "-" s
          roomname = concat $ init splitString
          (sectorid, checksum) = parseSectorIdChecksum $ last splitString

isRealRoom :: Room -> Bool
isRealRoom (Room n _ cs) = (deriveChecksum n) == cs

-- Answers

advent4_1 = foldr (+) 0 $ map (sectorId) $ filter (isRealRoom) $ map (parseString) input
